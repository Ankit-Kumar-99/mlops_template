...
if log_to_mlflow=='true':
    with mlflow.start_run(run_id=run_id):
        mlflow.log_artifacts(f"/Volumes/{config['mlops_catalog']}/{config['mlops_b_schema']}/{config['mlops_b_volume']}/{config['output_storage_path']}", artifact_path="datasets")

file_path = f"/Volumes/{config['mlops_catalog']}/{config['mlops_b_schema']}/{config['mlops_b_volume']}/{config['output_storage_path']}".split('/', 1)[1]
try:
