...
print(current_job_run_id)
dbutils.jobs.taskValues.set(key='current_job_run_id', value=current_job_run_id)
...
def start_mlflow_run(experiment_name: str):
    ...
    # Fetch the current job run ID from dbutils.widgets
    current_job_run_id = dbutils.widgets.get("current_run_id")
...
experiment_name = f"/Users/{dbutils.notebook.entry_point.getDbutils().notebook().getContext().userName().get()}/mlops_classification_framework_ex"
...
file_path = f"/Volumes/{config['mlops_catalog']}/{config['mlops_s_schema']}/{config['mlops_s_volume']}/{config['output_storage_path']}" + f"/{current_job_run_id}"
