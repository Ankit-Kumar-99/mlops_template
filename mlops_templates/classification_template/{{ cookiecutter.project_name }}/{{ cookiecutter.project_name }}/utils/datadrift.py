...
# Categorical Tests
def g_test(reference, target):
    """
    Performs the G-test for categorical data to compare observed and expected frequencies.
    ...
    - p_value (float): The p-value of the test.
    """
    table = pd.crosstab(reference, target)
    _, p_value, _, _ = chi2_contingency(table, lambda_='log-likelihood')
    return p_value
